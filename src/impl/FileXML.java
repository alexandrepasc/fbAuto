		...
		try {
			...
			if (doc_.hasChildNodes()) {
				...
				if (tempNode_.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (nList_.item(0).getNodeName().equals("total")) {
						searchStructure_ = new SearchStructure[Integer.valueOf(nList_.item(0).getTextContent())];
					}
					
					if ((nList_.getLength() - 1) == searchStructure_.length) {
						...
						for (int x = 1; x < nList_.getLength(); x++) {
							...
							if (nList_.item(x).getNodeType() == Node.ELEMENT_NODE) {
								...
								searchStructure_[x - 1] = new SearchStructure();

								Field[] structField_ = searchStructure_[x - 1].getClass().getDeclaredFields();
								
								structField_[0].set(searchStructure_[x - 1], nList_.item(x).getAttributes().item(0).getTextContent());
								
								for (int i = 0; i < subnList_.getLength(); i++) {
									
									structField_[i + 1].set(searchStructure_[x - 1], subnList_.item(i).getTextContent());
								}
							}
						}
					}
